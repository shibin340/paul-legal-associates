      ...
      <div id="stats" className="grid grid-cols-2 lg:grid-cols-4 bg-navy-deep border-b border-gold/20" role="region" aria-label="Firm statistics">
        {STATS.map((s, i) => (
          <AnimateIn
            key={s.label}
            delay={i * 0.1}
            className={`text-center py-10 px-4 flex flex-col justify-center items-center ${i < STATS.length - 1 ? "border-r border-gold/15" : ""}`}
          >
            <div className="font-serif text-[1.85rem] lg:text-[2.2rem] font-bold text-gold leading-none mb-2">
              {s.value}
            </div>
            <div className="text-2xs tracking-[0.18em] uppercase text-cream/50">
              {s.label}
            </div>
          </AnimateIn>
        ))}
